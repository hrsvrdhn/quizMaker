...
def analytics_download(request):
	...
	response['Content-Disposition'] = 'attachment; filename="{}-{}.csv"'.format(from_date.date(), to_date.date())

	writer = csv.writer(response)
	for obj in objs:
		try:
			writer.writerow([obj.user.user.user.username, obj.ip_address, obj.content_type, obj.content_object, obj.timestamp])
		except:
			pass
