...
urlpatterns = [
    ...
    re_path(
        ...
        views.updateQuestionForm,
        name="updateQuestion",
    ),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/add$", views.addQuestionForm, name="addQuestionForm"
    ),
    re_path(r"^ajax/add", views.addTestForm, name="addTestForm"),
    re_path(r"^ajax/(?P<pk>[0-9]+)/addtopic$", views.AddTopic, name="addtopic"),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/deletetopic$", views.DeleteTopic, name="deletetopic"
    ),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/questions$", views.getQuestionList, name="getQuestions"
    ),
    re_path(
        r"^ajax/(?P<qpk>[0-9]+)/response$",
        views.QuestionStatForm,
        name="QuestionStatForm",
    ),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/publish$", views.ToggleTestPublish, name="togglepublish"
    ),
    re_path(r"^ajax/(?P<pk>[0-9]+)/delete$", views.deleteTest, name="deleteTest"),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/activate$", views.ToggleTestActive, name="toggleactive"
    ),
    re_path(
        r"^ajax/(?P<pk>[0-9]+)/private$", views.ToggleTestPrivate, name="toggleprivate"
    ),
    ...
    re_path(
        r"^ajax/test/(?P<pk>[0-9]+)/add_comment$", views.addComment, name="add_comment"
    ),
    re_path(
