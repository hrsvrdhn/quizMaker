...
def following_email(follower_name, following_name, following_email):
    ...
    mail = {
        "personalizations": [
            {"to": [{"email": following_email}], "subject": "New follower"}
        ],
        "from": {"name": "QuizMaker", "email": "no-reply@quizmaker.herokuapp.com"},
        "content": [
            {
                "type": "text/html",
...
def web_feedback_email(instance):
    ...
    mail = {
        "personalizations": [
            {
                "to": [
                    {
                        "email": getattr(
                            settings, "DEFAULT_ADMIN_EMAIL", "no-reply@quizmaker.com"
                        )
                    }
                ],
        ...
        "from": {"name": instance.name, "email": instance.email},
        "content": [
            {
                "type": "text/html",
                "value": "<p>Name: {}</p><p>Email: {}</p><p>Description: {}</p>".format(
                    instance.name, instance.email, instance.description
                ),
            }
        ],
    }
...
def new_user_signup_email(name, email_address):
    ...
    mail = {
        "personalizations": [
            {"to": [{"email": email_address}], "subject": "Welcome to QuizMaker"}
        ],
        "from": {
            "name": name,
            "email": getattr(settings, "DEFAULT_ADMIN_EMAIL", "no-reply@quizmaker.com"),
        },
        "content": [{"type": "text/html", "value": new_user_message}],
    }
    response = sg.client.mail.send.post(request_body=mail)
    if response.status_code == 202:
        print("Email sent to admin")
